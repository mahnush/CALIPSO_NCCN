...
!-------------------------------------------------------------------------------
 ...
 !-------------------------------------------------------------------------------
 ! size distributions and aerosol densities 

!  ! MACC densities [g cm-3]
!  REAL(dp), PARAMETER :: dens_so4 = 1.769_dp, &      ! density SO4 
!		       dens_bc = 1.0_dp,    &	      ! density BC 
!		       dens_omh = 1.769_dp,  &	      ! density OM 
!		       dens_omn = 1.769_dp,  &	      ! density OM 
!		       dens_ss = 1.183_dp,  &	      ! density SS 
!		       dens_du = 2.61_dp	      ! density DU 
 
  ! CAMS densities [g cm-3]
  REAL(dp), PARAMETER :: dens_so4 = 1.8_dp, &         ! density SO4 
		       dens_bc = 1.0_dp,    &	      ! density BC 
		       dens_omh = 1.8_dp,  &	      ! density OM 
		       dens_omn = 2.0_dp,  &	      ! density OM 
		       dens_ss = 1.183_dp,  &	      ! density SS 
		       dens_du = 2.61_dp	      ! density DU 
 
  REAL(dp), PARAMETER :: aerdens(nmod) = (/dens_ss, dens_ss, dens_ss, &
  					   dens_du, dens_du, dens_du, &
					   dens_omn, dens_omh, dens_bc, dens_bc, dens_so4/)
 
!  ! MACC count median radius [microns]
...
!		       cmr_du3 = 0.29_dp 	              
  ...
  ! CAMS count median radius [microns]
  REAL(dp), PARAMETER :: cmr_so4 = 0.0212_dp, &       
		       cmr_bcn = 0.0118_dp,  &	     
		       cmr_bch = 0.0118_dp,  &
		       cmr_omn = 0.471_dp,  &
		       cmr_omh = 0.0212_dp,  &	     
...
!  ! MACC geometric standard deviation
...
!			gsd_du3  = 2.0_dp           
  ...
  ! CAMS geometric standard deviation
   REAL(dp), PARAMETER :: gsd_so4 = 2.24_dp, &	     
		       gsd_bc = 2.0_dp, &	   
		       gsd_omh = 2.24_dp, &
		       gsd_omn = 2.51_dp, &	   
		       gsd_ss1  = 1.9_dp, &
   ...
   REAL(dp), PARAMETER :: aergsd(nmod) = (/gsd_ss1, gsd_ss2, gsd_ss3, &
					   ...
					   gsd_omn, gsd_omh, gsd_bc, gsd_bc, gsd_so4/)
...
!references for kappa:

!Zhang et al., 2012, ECHAM-HAM2 -> ref: Petters & Kreideweiss(2007): 
!SS = 1.12, SU = 0.6
!POA = 0.06 (organic carbon), (SOA = 0.037 (secondary organics))
!Icon-A-Ham2.3(as Echam6.3-Ham2.3?): POA = SOA = 0.06
!Zieger et al 2017: sea salt = 1.1
!Icon-A-Ham2.3(as Echam6.3-Ham2.3?): POA = SOA = 0.06
!Poehlker at al (draft): POA = SOA = 0.12, SU = 0.63 

  REAL(dp), PARAMETER :: kappa(nmod) = (/1.1_dp,1.1_dp,1.1_dp, &
  					 0.0_dp,0.0_dp,0.0_dp, &
					 0.0_dp,0.06_dp,0.0_dp,0.06_dp,0.6_dp/)
