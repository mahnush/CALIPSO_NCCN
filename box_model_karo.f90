...
IMPLICIT NONE
  ...
  REAL(dp),ALLOCATABLE,DIMENSION(:,:,:)         :: sz
...
!--- get netCDF data parameters from reanalysis:  
  ...
  CALL netcdf_input(ifile1, ifile2, &
  		    ...
  		    SP, sz, press, temp, spechum, mixratio_SSs, mixratio_SSm, mixratio_SSl, &
...
!--- Allocate most of the Output:
  ...
  ALLOCATE(VarCCN_ss_015(nLon, nLat, nLev, nTimes))
  ALLOCATE(VarCCN_ss_025(nLon, nLat, nLev, nTimes))
...
!   VarMC(lo,la,ig,it) = -9999.99_dp
        ...
        VarCCN_ss_015(lo,la,ig,it) = -9999.99_dp
        VarCCN_ss_025(lo,la,ig,it) = -9999.99_dp
        !msh
...
!   VarMC(lo,la,ig,it) = SUM(mconc(kbdim,klev,:))
     ...
     VarCCN_ss_015(lo,la,ig,it) = ccnsum(kbdim,klev,7)
     VarCCN_ss_025(lo,la,ig,it) = ccnsum(kbdim,klev,10)
