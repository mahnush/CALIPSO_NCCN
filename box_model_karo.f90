PROGRAM box_model_karo
  ...
  USE mo_box_netcdf_io, 	ONLY: netcdf_dimensions, netcdf_input, netcdf_output_ccn
  USE mo_box_netcdf_io, 	ONLY: netcdf_output_ccnspec, netcdf_output_nconc, netcdf_output_mconc
  ...
  USE mo_ham_activ,		ONLY: ham_activ_koehler_ab, ham_activ_koehler_kappa
  USE mo_ham_activ,		ONLY: ham_activ_abdulrazzak_ghan
!-------------------------------------------------------------------------------

IMPLICIT NONE

  CHARACTER(len=150)	:: ifilename1, ifilename2
  CHARACTER(len=150)	:: ofilename1, ofilename2, ofilename3, ofilename4
  CHARACTER(len=:), ALLOCATABLE :: ifile1, ifile2, ofile1, ofile2, ofile3, ofile4
   
  CHARACTER(len=4)	:: year
  CHARACTER(len=2)	:: mon
  CHARACTER(len=2)      :: day
  INTEGER		:: nTimes, nLev, nLat, nLon, nhym, nhyi

  
  ! parameters from input file

  REAL(dp),ALLOCATABLE,DIMENSION(:)		:: time, lev, lat, lon, hyam, hybm, hyai, hybi
  REAL(dp),ALLOCATABLE,DIMENSION(:,:,:)         :: sz, sp
  REAL(dp),ALLOCATABLE,DIMENSION(:,:,:,:)	:: geop, press, temp, spechum
  ...
  REAL(dp),ALLOCATABLE,DIMENSION(:,:,:,:)       :: VarCCN_01, VarCCN_02, VarCCN_04
  REAL(dp),ALLOCATABLE,DIMENSION(:,:,:,:)       :: VarCCN_06, VarCCN_08, VarCCN_10

                                   				                                                                                                                   
  REAL(dp),ALLOCATABLE,DIMENSION(:,:,:,:)       :: VarCCN_01_SSs, VarCCN_01_SSm, VarCCN_01_SSl
  REAL(dp),ALLOCATABLE,DIMENSION(:,:,:,:)	:: VarCCN_01_OMh, VarCCN_01_BCh, VarCCN_01_SU
  ...
  REAL(dp),ALLOCATABLE,DIMENSION(:,:,:,:)	:: VarCCN_06_SSs, VarCCN_06_SSm, VarCCN_06_SSl
  REAL(dp),ALLOCATABLE,DIMENSION(:,:,:,:)	:: VarCCN_06_OMh, VarCCN_06_BCh, VarCCN_06_SU
  
  REAL(dp),ALLOCATABLE,DIMENSION(:,:,:,:)	:: VarCCN_08_SSs, VarCCN_08_SSm, VarCCN_08_SSl
  REAL(dp),ALLOCATABLE,DIMENSION(:,:,:,:)	:: VarCCN_08_OMh, VarCCN_08_BCh, VarCCN_08_SU
...
!--- read input statements:
  ...
  CALL read_input(ifilename1, ifilename2,&
  		  ofilename1, ofilename2, ofilename3, ofilename4, year, mon, day)
...
!--- get netCDF data dimensions from reanalysis:
  ...
  CALL netcdf_dimensions(ifile1, nTimes, nLev, nLat, nLon, nhym, nhyi)
...
!--msh
  ...
  CALL netcdf_input(ifile1, ifile2, nTimes, nLev, nLat, nLon, nhym, nhyi, &
  		    time, lev, lat, lon, hyam, hybm, hyai, hybi, &
  		    sp, sz, press, temp, spechum, &
		    mixratio_SSs, mixratio_SSm, mixratio_SSl, &
...
!--- Allocate most of the Output:
  ...
  ALLOCATE(VarCCN_01_SSs(nLon, nLat, nLev, nTimes))   
  ALLOCATE(VarCCN_01_SSm(nLon, nLat, nLev, nTimes))    
  ALLOCATE(VarCCN_01_SSl(nLon, nLat, nLev, nTimes))    
  ALLOCATE(VarCCN_01_OMh(nLon, nLat, nLev, nTimes))  
  ALLOCATE(VarCCN_01_BCh(nLon, nLat, nLev, nTimes))   
  ALLOCATE(VarCCN_01_SU(nLon, nLat, nLev, nTimes))
  ...
  ALLOCATE(VarCCN_06_SSs(nLon, nLat, nLev, nTimes))   
  ALLOCATE(VarCCN_06_SSm(nLon, nLat, nLev, nTimes))    
  ALLOCATE(VarCCN_06_SSl(nLon, nLat, nLev, nTimes))    
  ALLOCATE(VarCCN_06_OMh(nLon, nLat, nLev, nTimes))  
  ALLOCATE(VarCCN_06_BCh(nLon, nLat, nLev, nTimes))   
  ALLOCATE(VarCCN_06_SU(nLon, nLat, nLev, nTimes))

  ALLOCATE(VarCCN_08_SSs(nLon, nLat, nLev, nTimes))   
  ALLOCATE(VarCCN_08_SSm(nLon, nLat, nLev, nTimes))    
  ALLOCATE(VarCCN_08_SSl(nLon, nLat, nLev, nTimes))    
  ALLOCATE(VarCCN_08_OMh(nLon, nLat, nLev, nTimes))  
  ALLOCATE(VarCCN_08_BCh(nLon, nLat, nLev, nTimes))   
  ALLOCATE(VarCCN_08_SU(nLon, nLat, nLev, nTimes))
...
!msh
      ...
      vr=nlev-(ig-1)
      IF(vr .EQ. nlev) THEN
         geop(lo,la,vr,it) = sz(lo,la,it)/g	!geopotential height [gpm] at surface
	...
	!compute deltaz using the hypsometric equation, assuming dry atmosphere by substituting Tvirt with T
	!geopot. height (z) = gh(surf) - delta z
      ...
      !WRITE(*,*) 'temperature: ptm1 [K]', ptm1
      !WRITE(*,*) 'presssure: papm1 [Pa]', papm1   
      !WRITE(*,*) 'saturation water vapor pressure: pesw [Pa]', pesw	
      !WRITE(*,*) 'moist air density: prho [kg/m3]', prho
      !WRITE(*,*) 'dry air density: prho_dry [kg/m3]', prho_dry  	 
      !WRITE(*,*) 'specific humidity: pqm1 [kg/kg]', pqm1  
...
!be aware that hygroscopicity parameter B (zb) is now substituted
!by a fixed kappa value defined in mo_box_aerosols

      CALL ham_activ_koehler_kappa(kproma,   kbdim,   klev,  krow,  ktdia, &
                                pxtm1,    ptm1,    za,    zb            )
...
!--- CCn at fixed prescribed supersaturations:	
      ...
      !WRITE(*,*) 'ccn_02[1/m3]', ccnsum(:,:,2), ' ccn_10[1/m3]', ccnsum(:,:,4)
      
      !ccntest = cnfrac(kbdim,klev,3,11)*nconc(kbdim,klev,11)
      !WRITE(*,*) 'ccn_04_SU[1/m3]', ccntest, ' cnfrac_04_SU[%]', cnfrac(:,:,3,11)


!-------------------------------------------------------------------------------
!--- Activation Scheme:	
!--- CDNC for specific supersaturation depending on characteristic updraft

!not needed for CCN computations!
...
!     
      ...
      !DO iw = 1,nw	
      ! WRITE(*,*) 'iw', iw, 'pw [m/s]', pw(iw), 'max. ssat [% 0-1]', zsmax(:,:,iw)
      ! DO jclass = 1,nmod
      !  WRITE(*,*) 'jclass', jclass, 'zrc [nm]', zrc(:,:,jclass,iw)*10.**(9.)
      ! ENDDO	
      !ENDDO
     
      !WRITE(*,*) ' '
      !DO jclass = 1,nmod
      !  WRITE(*,*) 'jclass', jclass, 'znact [1/cm3]', znact(:,:,jclass)*10.**(-6.), &
      !  &'act.frac [%]', znact(:,:,jclass)*100._dp/nconc(:,:,jclass)
      !ENDDO   
      !WRITE(*,*) 'pcdncact [1/cm3]', pcdncact*10.**(-6.)
      !WRITE(*,*) 'act.frac total[%]', pcdncact*100._dp/SUM(nconc(:,:,:))
      !WRITE(*,*) 'act.frac hygro [%]', pcdncact*100._dp/nconc_hygro

      !WRITE(*,*) 'it', it, 'pcdncact [1/cm3]', pcdncact*10.**(-6.)	!for testing
...
!--- Collect output :	
	...
	VarCCN_02(lo,la,ig,it) = -9999.99_dp
        ...
        VarCCN_01_SSs(lo,la,ig,it) = -9999.99_dp
	VarCCN_01_SSm(lo,la,ig,it) = -9999.99_dp 
        VarCCN_01_SSl(lo,la,ig,it) = -9999.99_dp 
	VarCCN_01_OMh(lo,la,ig,it) = -9999.99_dp
	VarCCN_01_BCh(lo,la,ig,it) = -9999.99_dp  
	VarCCN_01_SU(lo,la,ig,it)  = -9999.99_dp
	...
	VarCCN_04_SU(lo,la,ig,it)  = -9999.99_dp
        ...
        VarCCN_06_SSs(lo,la,ig,it) = -9999.99_dp
	VarCCN_06_SSm(lo,la,ig,it) = -9999.99_dp 
        VarCCN_06_SSl(lo,la,ig,it) = -9999.99_dp 
	VarCCN_06_OMh(lo,la,ig,it) = -9999.99_dp
	VarCCN_06_BCh(lo,la,ig,it) = -9999.99_dp  
	VarCCN_06_SU(lo,la,ig,it)  = -9999.99_dp
	
        VarCCN_08_SSs(lo,la,ig,it) = -9999.99_dp
	VarCCN_08_SSm(lo,la,ig,it) = -9999.99_dp 
        VarCCN_08_SSl(lo,la,ig,it) = -9999.99_dp 
	VarCCN_08_OMh(lo,la,ig,it) = -9999.99_dp
	VarCCN_08_BCh(lo,la,ig,it) = -9999.99_dp  
	VarCCN_08_SU(lo,la,ig,it)  = -9999.99_dp
   ...
   ELSE    
	...
	VarCCN_01(lo,la,ig,it) = ccnsum(kbdim,klev,1)
	VarCCN_02(lo,la,ig,it) = ccnsum(kbdim,klev,2)
	VarCCN_04(lo,la,ig,it) = ccnsum(kbdim,klev,3)
	VarCCN_06(lo,la,ig,it) = ccnsum(kbdim,klev,4)
	VarCCN_08(lo,la,ig,it) = ccnsum(kbdim,klev,5)
	VarCCN_10(lo,la,ig,it) = ccnsum(kbdim,klev,6)

	VarCCN_01_SSs(lo,la,ig,it) = cnfrac(kbdim,klev,1,1)*nconc(kbdim,klev,1)    
	VarCCN_01_SSm(lo,la,ig,it) = cnfrac(kbdim,klev,1,2)*nconc(kbdim,klev,2)    
	VarCCN_01_SSl(lo,la,ig,it) = cnfrac(kbdim,klev,1,3)*nconc(kbdim,klev,3)
	VarCCN_01_OMh(lo,la,ig,it) = cnfrac(kbdim,klev,1,8)*nconc(kbdim,klev,8)
	VarCCN_01_BCh(lo,la,ig,it) = cnfrac(kbdim,klev,1,10)*nconc(kbdim,klev,10)
	VarCCN_01_SU(lo,la,ig,it)  = cnfrac(kbdim,klev,1,11)*nconc(kbdim,klev,11)	

	VarCCN_02_SSs(lo,la,ig,it) = cnfrac(kbdim,klev,2,1)*nconc(kbdim,klev,1)    
	VarCCN_02_SSm(lo,la,ig,it) = cnfrac(kbdim,klev,2,2)*nconc(kbdim,klev,2)    
	VarCCN_02_SSl(lo,la,ig,it) = cnfrac(kbdim,klev,2,3)*nconc(kbdim,klev,3)
	VarCCN_02_OMh(lo,la,ig,it) = cnfrac(kbdim,klev,2,8)*nconc(kbdim,klev,8)
	VarCCN_02_BCh(lo,la,ig,it) = cnfrac(kbdim,klev,2,10)*nconc(kbdim,klev,10)
	VarCCN_02_SU(lo,la,ig,it)  = cnfrac(kbdim,klev,2,11)*nconc(kbdim,klev,11)	

	VarCCN_04_SSs(lo,la,ig,it) = cnfrac(kbdim,klev,3,1)*nconc(kbdim,klev,1)    
	VarCCN_04_SSm(lo,la,ig,it) = cnfrac(kbdim,klev,3,2)*nconc(kbdim,klev,2)    
	VarCCN_04_SSl(lo,la,ig,it) = cnfrac(kbdim,klev,3,3)*nconc(kbdim,klev,3)
	VarCCN_04_OMh(lo,la,ig,it) = cnfrac(kbdim,klev,3,8)*nconc(kbdim,klev,8)
	VarCCN_04_BCh(lo,la,ig,it) = cnfrac(kbdim,klev,3,10)*nconc(kbdim,klev,10)
	VarCCN_04_SU(lo,la,ig,it)  = cnfrac(kbdim,klev,3,11)*nconc(kbdim,klev,11)	

	VarCCN_06_SSs(lo,la,ig,it) = cnfrac(kbdim,klev,4,1)*nconc(kbdim,klev,1)    
	VarCCN_06_SSm(lo,la,ig,it) = cnfrac(kbdim,klev,4,2)*nconc(kbdim,klev,2)    
	VarCCN_06_SSl(lo,la,ig,it) = cnfrac(kbdim,klev,4,3)*nconc(kbdim,klev,3)
	VarCCN_06_OMh(lo,la,ig,it) = cnfrac(kbdim,klev,4,8)*nconc(kbdim,klev,8)
	VarCCN_06_BCh(lo,la,ig,it) = cnfrac(kbdim,klev,4,10)*nconc(kbdim,klev,10)
	VarCCN_06_SU(lo,la,ig,it)  = cnfrac(kbdim,klev,4,11)*nconc(kbdim,klev,11)

	VarCCN_08_SSs(lo,la,ig,it) = cnfrac(kbdim,klev,5,1)*nconc(kbdim,klev,1)    
	VarCCN_08_SSm(lo,la,ig,it) = cnfrac(kbdim,klev,5,2)*nconc(kbdim,klev,2)    	
	VarCCN_08_SSl(lo,la,ig,it) = cnfrac(kbdim,klev,5,3)*nconc(kbdim,klev,3)
	VarCCN_08_OMh(lo,la,ig,it) = cnfrac(kbdim,klev,5,8)*nconc(kbdim,klev,8)
	VarCCN_08_BCh(lo,la,ig,it) = cnfrac(kbdim,klev,5,10)*nconc(kbdim,klev,10)
	VarCCN_08_SU(lo,la,ig,it)  = cnfrac(kbdim,klev,5,11)*nconc(kbdim,klev,11)	

	VarCCN_10_SSs(lo,la,ig,it) = cnfrac(kbdim,klev,6,1)*nconc(kbdim,klev,1)    
	VarCCN_10_SSm(lo,la,ig,it) = cnfrac(kbdim,klev,6,2)*nconc(kbdim,klev,2)	
	VarCCN_10_SSl(lo,la,ig,it) = cnfrac(kbdim,klev,6,3)*nconc(kbdim,klev,3)
	VarCCN_10_OMh(lo,la,ig,it) = cnfrac(kbdim,klev,6,8)*nconc(kbdim,klev,8)
	VarCCN_10_BCh(lo,la,ig,it) = cnfrac(kbdim,klev,6,10)*nconc(kbdim,klev,10)
	VarCCN_10_SU(lo,la,ig,it)  = cnfrac(kbdim,klev,6,11)*nconc(kbdim,klev,11)        
...
!--- Write output file:	
 ...
 CALL netcdf_output_ccn(ofile1, year, mon, day, &
                        nTimes, nLev, nLat, nLon, nhym, nhyi, &
                        time, lev, lat, lon, hyam, hybm, hyai, hybi, &
                        sp, sz, geop, &
 ...
 CALL netcdf_output_ccnspec(ofile2, year, mon, day, &
                        nTimes, nLev, nLat, nLon, nhym, nhyi, &
                        time, lev, lat, lon, hyam, hybm, hyai, hybi, &
                        sp, sz, geop, &
 ...
 CALL netcdf_output_nconc(ofile3, year, mon, day, &
                        nTimes, nLev, nLat, nLon, nhym, nhyi, &
                        time, lev, lat, lon, hyam, hybm, hyai, hybi, &
                        sp, sz, geop, VarNC, &
 ...
 CALL netcdf_output_mconc(ofile4, year, mon, day, &
                        nTimes, nLev, nLat, nLon, nhym, nhyi, &
                        time, lev, lat, lon, hyam, hybm, hyai, hybi, &
                        sp, sz, geop, VarMC, &
