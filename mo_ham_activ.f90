...
!K   PUBLIC ham_avail_activ_lin_leaitch, ham_activ_diag_lin_leaitch
  PUBLIC ham_activ_koehler_ab, ham_activ_koehler_kappa
...
!K        b(jclass)%ptr(1:kproma,:,krow)=pb(1:kproma,:,jclass)
  ...
  SUBROUTINE ham_activ_koehler_kappa(kproma,   kbdim,   klev,  krow,  ktdia, &
                                  pxtm1,    ptm1,    pa,    pb            )


    ! *ham_activ_koehler_ab* calculates the Koehler A and B coefficients using kappa
    
    USE mo_constants,   ONLY: rhoh2o, argas, amw, zsten
    USE mo_box_aerosols,   ONLY: nmod, ntrac, aersol, kappa	
    ...
    !---for calculation of curvature parameter A 
    zafac = 2._dp*zsten*zamw / (rhoh2o*argas)

    DO jclass=1,nmod  
      lsoluble = aersol(jclass)     
    	IF (lsoluble) THEN

	!--- 1.1.1) Hygroscopicity parameter B 
	pb(1:kproma,:,jclass)= kappa(jclass)
  ...
  END SUBROUTINE ham_activ_koehler_kappa
